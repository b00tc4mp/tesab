...
function Header() {
  ...
  const [toggleEquipmentButtonText, setToggleEquipmentButton] = useState('arrow_drop_down')
  const [toggleServicesButtonText, setToggleServicesButton] = useState('arrow_drop_down')
  ...
  return <header className='z-10 absolute top-0 w-full'>
    <div className="flex justify-between bg-[#323A47] p-2">
      ...
      <nav className="flex space-x-5">
        <span className="flex items-center space-x-2">
          <img className="w-4" src="images/social/Telefono-Blanco.png" alt="Telephone" onClick={togglePhoneNumber} />
          {phoneNumberVisible && <a href="tel:+34944924003" className="text-white text-xs">944 924 003</a>}
        </span>

        <span className="flex items-center space-x-2">
          <img className="w-4" src="images/social/Correo-blanco.png" alt="E-mail" onClick={toggleEmail} />
          {emailVisible && <a href="mailto:tesab@tesabspain.es" className="text-white text-xs">tesab@tesabspain.es</a>}
        </span>
      </nav>
    </div>
